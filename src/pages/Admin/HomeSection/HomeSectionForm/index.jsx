...
// Re-added dnd-kit imports for the banner section
...
// Component Chip đơn giản, không có logic kéo thả
function StaticChip({ item, onDelete }) {
  const handleDelete = () => {
    onDelete(item.id);
  };
...
// Component Banner có logic kéo thả
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={4}>
          <Stack spacing={1.5}>
            ...
            >
              <input
                ...
                onChange={(e) => {
                  ...
                  // clear imageUrl cũ
                  setValue(`banners.${idx}.imageUrl`, null);
                }}
              />
            ...
            {errors?.banners?.[idx]?.imageFile && <FormHelperText error>{errors.banners[idx].imageFile.message}</FormHelperText>}
...
export default function HomeSectionFormPage() {
  ...
  useEffect(() => {
    if (errors.banners && watchedBanners.length > 0) {
      clearErrors('banners');
    }
  }, [watchedBanners, errors.banners, clearErrors]);
  ...
  const onSubmit = async (data) => {
    if (['productWithBanner', 'full'].includes(data.type)) {
      if (!data.banners || data.banners.length === 0) {
        setError('banners', {
          type: 'manual',
          message: 'Cần thêm ít nhất 1 banner!'
        });
        toast.error('Cần thêm ít nhất 1 banner!');
        return;
      }

      const hasValidImage = data.banners.some((b) => b.imageFile || b.imageUrl || b.previewUrl);
      if (!hasValidImage) {
        setError('banners', {
          type: 'manual',
          message: 'Mỗi banner cần có ảnh!'
        });
        toast.error('Mỗi banner cần có ảnh!');
        return;
      }
    }
    ...
    try {
      ...
      const totalBannerCount = bannersMeta.filter((b) => b.existingImageUrl || b.hasNewFile).length;
      ...
      data.banners?.forEach((b) => {
        if (b.imageFile) {
          console.log('=== DEBUG append file ===', b.imageFile.name);
                      ...
                      {selectedCategoryObjects.length > 0 && (
                        <Stack direction="row" flexWrap="wrap" sx={{ gap: 1, alignItems: 'flex-start' }} mt={1.5}>
                          {(field.value || []).map((catId) => {
                            ...
                            if (!cat) return null;
                            return (
                              <StaticChip key={String(cat.id)} item={{ ...cat, thumbnail: null }} onDelete={() => handleDelete(cat.id)} />
                            );
                          })}
                        </Stack>
                      )}
            ...
            <FormControl fullWidth error={!!errors.productIds}>
              <Controller
                ...
                render={({ field }) => {
                  ...
                  const selectedProductObjects = (field.value || [])
                    .map((id) => productOptions.find((opt) => opt.id === id))
                    .filter(Boolean);
                  ...
                  return (
                    <>
                      ...
                      {errors.productIds && <FormHelperText error>{errors.productIds.message}</FormHelperText>}
