...
const BrandList = () => {
  ...
  const fetchBrands = async () => {
    try {
      const res = await brandService.getAll({ page, limit, search, status });
  ...
  const handleDragEnd = async ({ active, over }) => {
    if (active.id !== over?.id) {
      ...
      try {
        ...
        Toastify.success('Đã lưu thứ tự hiển thị');
      } catch (err) {
        console.error('❌ Lỗi cập nhật thứ tự:', err);
        Toastify.error('Cập nhật thứ tự thất bại');
      }
    }
  };
  ...
  const handleBulkAction = async () => {
    try {
      if (bulkAction === 'delete') {
        ...
        await brandService.restore(selectedIds);
        Toastify.success('Đã khôi phục');
      } else if (bulkAction === 'forceDelete') {
        await brandService.forceDelete(selectedIds);
