...
const StatusChip = ({ status }) => <Chip label={labelMap[status] || status} color={statusColors[status] || 'default'} size="small" />;
    ...
    <Box sx={{ mt: 4 }}>
      ...
      {totalItems > itemsPerPage && (
        <Box sx={{ mt: 2, display: 'flex', justifyContent: 'center' }}>
          <MUIPagination currentPage={page} totalItems={totalItems} itemsPerPage={itemsPerPage} onPageChange={setPage} />
        </Box>
      )}
      ...
      <Dialog open={openRejectDialog} onClose={handleCloseRejectDialog}>
          ...
          <FormControl component="fieldset" error={rejectReasonError} fullWidth>
            ...
            >
              {rejectReasonsOptions.map((option) => (
                <FormControlLabel key={option.id} value={option.id} control={<Radio size="small" />} label={option.label} />
              ))}
            </RadioGroup>
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={!selectedRejectReasonOption || (selectedRejectReasonOption === 'OTHER' && !customRejectReason.trim())}
          >
            Xác nhận từ chối
          </Button>
        </DialogActions>
