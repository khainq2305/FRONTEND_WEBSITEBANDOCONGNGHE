...
const handleSubmit = async (e) => {
  ...
  Object.entries(formData).forEach(([k, v]) => {
    if (v instanceof File) {
      ...
      const isRequiredText = ['siteName','siteDescription','address'].includes(k);
      if (typeof v === 'string' && !isRequiredText && v.trim() === '') return;
      payload.append(k, typeof v === 'boolean' ? String(v) : v);
    }
  });

  try {
    const updated = await systemSettingService.update(payload); 
    ...
    setFormData({
      ...
      hotlineSales: updated.hotlineSales || '',
      hotlineWarranty: updated.hotlineWarranty || '',
      hotlineFeedback: updated.hotlineFeedback || '',
      emailContact: updated.emailContact || '',
      siteLogo: updated.siteLogo || null,
      favicon: updated.favicon || null,
      facebookEnabled: updated.facebookEnabled ?? true,
      instagramEnabled: updated.instagramEnabled ?? true,
      tiktokEnabled: updated.tiktokEnabled ?? true,
      youtubeEnabled: updated.youtubeEnabled ?? true,
      zaloEnabled: updated.zaloEnabled ?? true,
      showSocialFooter: updated.showSocialFooter ?? true,
      facebookPageUrl: updated.facebookPageUrl || '',
