...
const ProductForm = ({ onSubmit, initialData }) => {
  const [formData, setFormData] = useState({
    ...
    variants: [],
    skus: [],
    productSpecs: [{ key: '', value: '', sortOrder: 0 }]
  });
      ...
      try {
        ...
        if (parsed.formData) {
          setFormData(prev => ({
            ...prev,
            ...parsed.formData,
            productSpecs: Array.isArray(parsed.formData.productSpecs) && parsed.formData.productSpecs.length > 0
  ...
  useEffect(() => {
    if (!formData.hasVariants && formData.name && formData.skus.length === 1 && formData.skus[0] && !(formData.skus[0].skuCode || '').trim()) {
  ...
  const handleProductSpecChange = (specIndex, field, value) => {
    setFormData(prev => {
      const updatedProductSpecs = [...(Array.isArray(prev.productSpecs) && prev.productSpecs.length > 0 ? prev.productSpecs : [{ key: '', value: '', sortOrder: 0 }])];
      if (updatedProductSpecs[specIndex]) {
        updatedProductSpecs[specIndex] = { ...updatedProductSpecs[specIndex], [field]: value };
      }
  ...
  const addProductSpec = () => {
    setFormData(prev => {
      ...
      return {
        ...
        productSpecs: [
          ...currentSpecs,
          { key: '', value: '', sortOrder: currentSpecs.length > 0 ? Math.max(0, ...currentSpecs.map(s => Number(s.sortOrder || 0))) + 1 : 0 }
        ]
      };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payloadForJson = {
      ...
      description: formData.description.trim(), // <-- Thêm description vào payload chính
      thumbnail: "",
      orderIndex:
        formData.orderIndex === "" || isNaN(Number(formData.orderIndex))
          ? ""
          : parseInt(formData.orderIndex, 10),
    ...
    if (formData.hasVariants) {
      payloadForJson.variants = productConfiguredVariants
        ...
        .map((pvc) => {
          const variantTypeDetails = availableVariants.find(
            (av) => av.id === pvc.variantTypeId
          );
          return {
            id: pvc.variantTypeId,
            name: pvc.variantTypeName,
            values: pvc.applicableValueIds.map((valueId) => {
              const valueDetails = variantTypeDetails?.values.find(
                (v) => v.id === valueId
              );
    ...
    payloadForJson.skus = formData.skus.map((sku) => ({
      ...
      originalPrice:
        sku.originalPrice == null || sku.originalPrice === ""
          ? ""
          : isNaN(Number(sku.originalPrice))
            ? ""
      ...
      stock:
        sku.stock == null || sku.stock === ""
          ? ""
          : isNaN(Number(sku.stock))
            ? ""
            : Number(sku.stock),
      height:
        sku.height == null || sku.height === ""
          ? ""
          : isNaN(Number(sku.height))
            ? ""
            : Number(sku.height),
      width:
        sku.width == null || sku.width === ""
          ? ""
          : isNaN(Number(sku.width))
            ? ""
            : Number(sku.width),
      length:
        sku.length == null || sku.length === ""
          ? ""
          : isNaN(Number(sku.length))
            ? ""
            : Number(sku.length),
      weight:
        sku.weight == null || sku.weight === ""
          ? ""
          : isNaN(Number(sku.weight))
            ? ""
            : Number(sku.weight),
      mediaUrls: sku.mediaUrls || [],
      // <-- Đã xóa description khỏi payload của SKU
    ...
    if (!formData.hasVariants && media.length > 0 && formData.skus.length > 0) {
        media.forEach((m, index) => {
            if (m.file instanceof File) {
                ...
                payloadForJson.skus[0].mediaUrls.push(m.url);
            }
        });
        ...
        formData.skus.forEach((sku, skuIndex) => {
            ...
            filesForThisSku.forEach((item) => {
                if (item.file instanceof File) {
                    ...
                    payloadForJson.skus[skuIndex].mediaUrls.push(item.url);
                }
            });
        });
    }
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setFormData({
        ...
        variants: initialProductVariantsData || [],
        skus: processedSkus,
        productSpecs: (initialProductSpecs && initialProductSpecs.length > 0)
      ...
      setThumbnail(initialProductThumbnail
        ? { url: initialProductThumbnail.startsWith('http') || initialProductThumbnail.startsWith('blob:') ? initialProductThumbnail : `${API_BASE_URL}${initialProductThumbnail}` }
        : null
      );
    ...
    <form onSubmit={handleSubmit}>
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={8}>
          ...
          {formErrors.shortDescription && <FormHelperText error>{formErrors.shortDescription}</FormHelperText>}
          ...
          <Typography variant="subtitle1" sx={{ mt: 3, mb: 1 }}>Thông tin thêm (VD: Hướng dẫn sử dụng)</Typography>
                ...
                {(provided) => (
                  <div {...provided.droppableProps} ref={provided.innerRef}>
                    {(formData.productSpecs || []).map((spec, specIndex) => (
                      <Draggable key={`product-spec-key-${specIndex}`} draggableId={`product-spec-draggable-${specIndex.toString()}`} index={specIndex}>
                        {(providedDraggable) => (
                          <Box
                            ref={providedDraggable.innerRef}
                            {...providedDraggable.draggableProps}
                            sx={{ p: 1.5, border: '1px dashed #e0e0e0', borderRadius: 0.5, mb: 1.5, background: '#fdfdfd', display: 'flex', alignItems: 'center' }}
                          >
                            <Box {...providedDraggable.dragHandleProps} sx={{ display: 'flex', alignItems: 'center', cursor: 'grab', mr: 1, color: '#757575' }}>
                              <DragIndicatorIcon />
                            </Box>
                            <Grid container spacing={1} alignItems="center" sx={{ flexGrow: 1 }}>
                              <Grid item xs={12} sm={4}>
                                <TextField fullWidth label="Tên thông số" value={spec.key} onChange={(e) => handleProductSpecChange(specIndex, 'key', e.target.value)} size="small" error={!!formErrors.productSpecs?.[specIndex]?.key} helperText={formErrors.productSpecs?.[specIndex]?.key} />
                              </Grid>
                              <Grid item xs={12} sm={4}>
                                <TextField fullWidth label="Giá trị" value={spec.value} onChange={(e) => handleProductSpecChange(specIndex, 'value', e.target.value)} size="small" error={!!formErrors.productSpecs?.[specIndex]?.value} helperText={formErrors.productSpecs?.[specIndex]?.value} />
                              </Grid>
                              <Grid item xs={12} sm={2.5}>
                                <TextField fullWidth type="number" label="Thứ tự" value={spec.sortOrder || 0} InputProps={{ readOnly: true }} size="small" />
                              </Grid>
                              <Grid item xs={12} sm={1.5} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                                <IconButton onClick={() => removeProductSpec(specIndex)} color="error" aria-label="Xóa thông số" disabled={(formData.productSpecs || []).length <= 1 && !spec.key && !spec.value}>
                                  <DeleteIcon />
                                </IconButton>
                              </Grid>
                            </Grid>
        ...
        <Grid item xs={12}>
          <FormControlLabel control={<Switch checked={!!formData.hasVariants} onChange={(event) => {
            ...
            setFormData(prev => {
              ...
              if (newHasVariants) {
                newSkus = prev.skus.length > 0 && !prev.hasVariants && prev.skus[0]
                  ? [{ ...createEmptySku(), skuCode: prev.skus[0].skuCode }]
                  : (prev.skus.length === 0 ? [createEmptySku()] : []);
                ...
                if (prev.skus.length > 0 && skuMediaFiles[0] && skuMediaFiles[0].length > 0) {
                  ...
                  setMedia(prev.skus[0].mediaUrls.map(url => ({ url, type: getFrontendFileType(url), id: Date.now() + Math.random() })));
                } else {
                  setMedia([]);
                }
              }
          ...
          <Alert severity={formData.hasVariants ? 'info' : 'warning'} sx={{ mt: 1, mb: 2 }}>
            {formData.hasVariants ? 'Sản phẩm có nhiều phiên bản. Mỗi phiên bản (SKU) sẽ có thông tin giá, kho, vận chuyển, và media riêng.' : 'Sản phẩm chỉ có một phiên bản duy nhất. Nhập thông tin giá, kho, media... ở bên dưới.'}
          </Alert>
        </Grid>
        ...
        {formData.hasVariants && (
          <Grid item xs={12}>
                        ...
                        <Select
                          ...
                          onChange={(e) => {
                            ...
                            setProductConfiguredVariants((prev_1) => {
                              ...
                              updated_1[pvcIndex].applicableValueIds = newSelectedValueIds.filter(id => id !== `add_new_value_for_${pvc.variantTypeId}` && id !== "");
                              return updated_1;
                            });
                          }}
            ...
            <Button variant="outlined"
              onClick={() => {
                if (productConfiguredVariants.length === 0 || productConfiguredVariants[productConfiguredVariants.length - 1].variantTypeId) {
        ...
        <Grid item xs={12}>
          {!formData.hasVariants && formData.skus[0] && (
            <Box key={`sku-item-novariant-0`} sx={{ mb: 3, p: 2, border: '1px solid #e0e0e0', borderRadius: 1, background: '#fff' }}>
              <Typography variant="h6" gutterBottom>Thông tin Phiên bản duy nhất</Typography>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6} md={3}><TextField fullWidth label="Mã SKU (Sản phẩm)" value={formData.skus[0].skuCode || ''} onChange={(e) => handleSkuChange(0, 'skuCode', e.target.value)} sx={{ mb: 2 }} error={!!formErrors.skus?.[0]?.skuCode} helperText={formErrors.skus?.[0]?.skuCode || ''} /></Grid>
                <Grid item xs={12} sm={6} md={3}><TextField fullWidth type="number" label="Giá gốc" value={formData.skus[0].originalPrice || ''} onChange={(e) => handleSkuChange(0, 'originalPrice', e.target.value)} sx={{ mb: 2 }} error={!!formErrors.skus?.[0]?.originalPrice} helperText={formErrors.skus?.[0]?.originalPrice || ''} InputProps={{ inputProps: { min: 0 } }} /></Grid>
                ...
                <Grid item xs={12} sm={6} md={3}><TextField fullWidth type="number" label="Tồn kho" value={formData.skus[0].stock || ''} onChange={(e) => handleSkuChange(0, 'stock', e.target.value)} sx={{ mb: 2 }} error={!!formErrors.skus?.[0]?.stock} helperText={formErrors.skus?.[0]?.stock || ''} InputProps={{ inputProps: { min: 0 } }} /></Grid>

                <Grid item xs={12}>
                  <Typography variant="subtitle1" sx={{ mb: 1, mt: 1, fontWeight: 'medium' }}>Thông tin Vận chuyển</Typography>
                </Grid>
                <Grid item xs={12} sm={6} md={3}><TextField fullWidth type="number" label="Rộng (cm)" value={formData.skus[0].width || ''} onChange={(e) => handleSkuChange(0, 'width', e.target.value)} sx={{ mb: 2 }} error={!!formErrors.skus?.[0]?.width} helperText={formErrors.skus?.[0]?.width || ''} InputProps={{ inputProps: { min: 0 } }} /></Grid>
                <Grid item xs={12} sm={6} md={3}><TextField fullWidth type="number" label="Cao (cm)" value={formData.skus[0].height || ''} onChange={(e) => handleSkuChange(0, 'height', e.target.value)} sx={{ mb: 2 }} error={!!formErrors.skus?.[0]?.height} helperText={formErrors.skus?.[0]?.height || ''} InputProps={{ inputProps: { min: 0 } }} /></Grid>
                <Grid item xs={12} sm={6} md={3}><TextField fullWidth type="number" label="Dài (cm)" value={formData.skus[0].length || ''} onChange={(e) => handleSkuChange(0, 'length', e.target.value)} sx={{ mb: 2 }} error={!!formErrors.skus?.[0]?.length} helperText={formErrors.skus?.[0]?.length || ''} InputProps={{ inputProps: { min: 0 } }} /></Grid>
                <Grid item xs={12} sm={6} md={3}><TextField fullWidth type="number" label="Nặng (kg)" value={formData.skus[0].weight || ''} onChange={(e) => handleSkuChange(0, 'weight', e.target.value)} sx={{ mb: 2 }} error={!!formErrors.skus?.[0]?.weight} helperText={formErrors.skus?.[0]?.weight || ''} InputProps={{ inputProps: { min: 0 } }} /></Grid>

                <Grid item xs={12}>
                  <Typography variant="subtitle1" sx={{ mb: 1 }}>Ảnh/Video sản phẩm</Typography>
                  <FormControl fullWidth error={!!formErrors.skus?.[0]?.mediaUrls} sx={{ mb: 0 }}>
                    <MediaUpload files={media} onChange={(files) => setMedia(files)} />
                    {formErrors.skus?.[0]?.mediaUrls && !Array.isArray(formErrors.skus[0].mediaUrls) && (<FormHelperText error>{formErrors.skus[0].mediaUrls}</FormHelperText>)}
          ...
          {formData.hasVariants && (
            <>
              <Typography variant="h6" gutterBottom sx={{ mt: 2, display: canManageVariantSkus ? 'block' : 'none' }}>Danh sách các Phiên bản (SKU)</Typography>
              <Tooltip title={!canManageVariantSkus ? "Hoàn tất cấu hình các loại thuộc tính và chọn giá trị cho chúng trước khi thêm SKU." : "Thêm phiên bản sản phẩm mới"}>
                <span>
                  <Button onClick={addSku} variant="outlined" sx={{ mb: 2, mt: productConfiguredVariants.length === 0 && formData.skus.length === 0 ? 2 : 0, display: canManageVariantSkus || (productConfiguredVariants.length === 0 && formData.skus.length === 0) ? 'inline-flex' : 'none' }}
                    disabled={!canManageVariantSkus && productConfiguredVariants.length > 0 || formData.skus.length >= 50} > + Thêm Phiên bản (SKU) </Button>
                </span>
              </Tooltip>
                  ...
                  <Grid container spacing={2}>
                    {productConfiguredVariants.filter(pvc => pvc.variantTypeId && pvc.applicableValueIds.length > 0).map((pvc) => (
                      <Grid item xs={12} key={pvc.variantTypeId}>
                        <TextField
                          select fullWidth label={pvc.variantTypeName}
                        ...
                        >
                          ...
                          {availableVariants.find(av => av.id === pvc.variantTypeId)?.values
                            ...
                            .map(valOpt => (<MenuItem key={valOpt.id} value={valOpt.id}>{valOpt.value}</MenuItem>))
                          }
                        </TextField>
                      </Grid>
                    ))}

                    <Grid item xs={12} sm={6} md={3}><TextField fullWidth label="Mã SKU" value={sku.skuCode || ''} onChange={(e) => handleSkuChange(i, 'skuCode', e.target.value)} sx={{ mb: 2 }} error={!!formErrors.skus?.[i]?.skuCode} helperText={formErrors.skus?.[i]?.skuCode || ''} /></Grid>
                    ...
                    <Grid item xs={12}>
                      <Typography variant="subtitle2" sx={{ mb: 1, mt: 1, fontWeight: 'medium' }}>Thông tin Vận chuyển</Typography>
                    </Grid>
                    ...
                    <Grid item xs={12}>
                      <Typography variant="subtitle2" sx={{ mb: 1 }}>Ảnh/Video cho phiên bản này (SKU {i + 1})</Typography>
                      <FormControl fullWidth error={!!formErrors.skus?.[i]?.mediaUrls} sx={{ mb: 0 }}>
                        <MediaUpload
                          key={`sku-media-${i}`}
                          files={skuMediaFiles?.[i]?.map((file) => ({
                            ...file,
                            url: file.url?.startsWith('http') || file.url?.startsWith('blob:') ? file.url : `${API_BASE_URL}${file.url}`,
                            type: file.type || getFrontendFileType(file.url)
                          })) || []}
                          onChange={(files) => handleMediaChangeForVariantSku(i, files)}
                        />
                         {formErrors.skus?.[i]?.mediaUrls && !Array.isArray(formErrors.skus[i].mediaUrls) && (<FormHelperText error>{formErrors.skus[i].mediaUrls}</FormHelperText>)}
                      </FormControl>
                    </Grid>
                  </Grid>
                </Box>
