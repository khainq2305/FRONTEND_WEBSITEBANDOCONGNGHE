...
const ProductForm = ({ onSubmit, initialData }) => {
    ...
    const createEmptySku = useCallback(() => ({
        id: null, skuCode: '', originalPrice: '', price: '', stock: '',
    ...
    const [formData, setFormData] = useState({
        ...
        orderIndex: 0, isActive: true, categoryId: '', brandId: '',
        hasVariants: false, 
        variants: [], skus: [createEmptySku()],
 ...
 const availableSpecGroups = useMemo(() => {
        const groups = (formData.productSpecs || [])
            .map(spec => spec.specGroup?.trim()) 
            .filter(Boolean);                   
                ...
                if (type === 'variantType' && pvcIndex !== undefined) {
                    ...
                    if (newType) {
                        setProductConfiguredVariants((prev) => {
                            const updated = [...prev];
                            updated[pvcIndex] = { ...updated[pvcIndex], variantTypeId: newType.id, variantTypeName: newType.name, applicableValueIds: [] };
                            return updated;
                        });
                    }
    ...
    const addSku = () => {
        if (formData.skus.length > 1 && productConfiguredVariants.some(pvc => !pvc.variantTypeId || pvc.applicableValueIds.length === 0)) {
            setFormErrors(prev => ({ ...prev, form: 'Hoàn tất cấu hình các loại thuộc tính và chọn giá trị cho chúng trước khi thêm SKU mới.' }));
            return;
        }
    ...
    const removeSku = (indexToRemove) => {
        ...
        setSkuMediaFiles(prevSkuMedia => {
            ...
            let newIndex = 0;
            Object.keys(newSkuMedia).sort((a, b) => a - b).forEach(oldIndex => {
    ...
    const handleSaveNewVariantValue = async () => {
        ...
        setIsSavingVariantValue(true);
        try {
            const res = await variantValueService.createQuick({ value: newVariantValueInput.trim(), variantId: currentVariantTypeForNewValue.id });
    ...
    const handleSubmit = async (e) => {
        ...
        payloadForJson.skus = formData.skus.map((sku) => ({
            ...
            variantValueIds: formData.hasVariants ? (Object.values(sku.variantSelections || {}).filter((vId) => vId !== '' && vId != null).map(Number)) : []
        }));
    ...
    useEffect(() => {
        productService.getCategoryTree().then(res => setCategoryTree(res.data.data || [])).catch(console.error);
        productService.getBrandList().then(res => setBrandList(res.data.data || [])).catch(console.error);
    }, []);
...
useEffect(() => {
    if (initialData) {
        ...
        setFormData(prev => ({
            ...
            skus: processedSkus, 
            productSpecs: (specs && specs.length > 0) ? specs.map(s => ({ key: s.key, value: s.value, specGroup: s.specGroup, sortOrder: s.sortOrder })) : [{ key: '', value: '', sortOrder: 0 }],
        }));
        ...
        if (hasVariants && variants && variants.length > 0) {
            setProductConfiguredVariants(variants.map(v => ({ 
                variantTypeId: v.id, 
    ...
    const canManageVariantSkus = productConfiguredVariants.length > 0 &&
