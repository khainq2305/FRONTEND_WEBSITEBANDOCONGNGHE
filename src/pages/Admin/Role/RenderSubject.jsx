...
const RenderSubject = (props) => {
  ...
  return (
    <>
      {subjectsToRender.map(subject => {
        ...
        const viewChecked = !!matrix[selectedRole]?.['view']; // trạng thái checkbox "view"
  ...
  const isViewAction = actionObj.action === 'read';
  const viewChecked = !!matrix[selectedRole]?.['read']; // trạng thái checkbox "view"
  ...
  return (
    ...
    >
      <Checkbox
  disabled={isAdminRole || (!isViewAction && !matrix[selectedRole]?.['read'])} // disabled nếu read chưa tick
  checked={isAdminRole ? true : !!matrix[selectedRole]?.[actionObj.action]}
  onChange={e =>
    handleUpdatePermission(selectedRole, subject.key, actionObj.action, e.target.checked)
  }
/>
