...
import './TestResponsiveSlider.css'; // Đảm bảo file CSS này không ghi đè những thứ quan trọng
...
// 👉 Lấy giá để hiển thị chính xác: nếu upcoming thì lấy giá sale (nếu có), ngược lại dùng giá hiện tại
const rawDisplayPrice = saleStatus === 'upcoming'
  ? (flashSaleInfo?.salePrice ?? currentPriceNum)
  : currentPriceNum;

// 👉 Nếu là sắp mở bán thì không hiển thị giá gạch ngang
...
// 👉 Dùng format làm mờ nếu chưa mở bán, còn lại hiển thị giá thật
...
const HorizontalProductSlider = ({ productsInput = [], imageBannerUrl, targetCountdownTime, countdownMode, bgColor = '#007BFF', saleStatus }) => {
    ...
    // Giữ nguyên logic này cho việc tính toán mặc định,
    // nhưng các breakpoint trong `responsive` sẽ là ưu tiên.
    const getIdealSlidesToShow = () => {
        if (windowWidth >= 1279) return 5;
        // iPad Pro (1024px)
        if (windowWidth >= 1024) return 4; // Thay đổi từ 3 -> 4 hoặc 5 để card đỡ dài
    ...
    const sliderSettings = {
        ...
        responsive: [
            {
                breakpoint: 1279, // Desktop nhỏ hơn 1280px
                settings: {
            ...
            // Đây là phần chính cần điều chỉnh cho iPad Pro và các thiết bị tương tự
            {
                breakpoint: 1024, // Thêm breakpoint cho iPad Pro ngang (thường 1024px) và các tablet lớn
                settings: {
                    slidesToShow: Math.min(4, totalProducts), // Tăng lên 4 sản phẩm để card ngắn hơn
                    slidesToScroll: 1,
                    infinite: totalProducts > 4,
                    arrows: totalProducts > 4 // Hiển thị mũi tên nếu có hơn 4 sản phẩm
                }
            },
            {
                breakpoint: 768, // iPad Mini (768px) và Tablet nhỏ hơn
                settings: {
                    slidesToShow: Math.min(3, totalProducts), // Giữ nguyên 3 sản phẩm
                    slidesToScroll: 1,
            ...
            {
                breakpoint: 479, // Mobile ngang (xs)
                settings: {
                    slidesToShow: Math.min(2, totalProducts), // Giữ nguyên 2 sản phẩm
                    slidesToScroll: 1,
            ...
            {
                breakpoint: 0, // Mobile đứng (mặc định)
                settings: {
                    slidesToShow: Math.min(1, totalProducts), // Giữ nguyên 1 sản phẩm
                    slidesToScroll: 1,
                ...
                <div className="two-row-slider-wrapper product-showcase-area group p-2 pt-14 relative" style={{ backgroundColor: bgColor }}>
                    ...
                    <div className="slider-inner-content">
                        {!productsInput || productsInput.length === 0 ? (
                            ...
                            // Phần này render khi số sản phẩm ít hơn slidesToShow, không dùng slick slider
                            ...
                            <Slider {...sliderSettings} ref={sliderRef} className="horizontal-slick-slider">
                                {productsInput.map((product) => (
                                    // px-1 để tạo khoảng cách giữa các slide
