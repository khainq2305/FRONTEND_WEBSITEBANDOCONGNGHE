...
const RecommendedProductsSection = ({ userId, currentProductId = null }) => {
    ...
    const renderStars = (rate, productIdForId) => {
        ...
        for (let i = 1; i <= 5; i++) {
            if (numRating >= i) stars.push(<FaStar key={`star-${i}-${productIdForId}`} className="text-yellow-400 text-[10.5px] sm:text-[11.5px]" />);
    ...
    const renderBadge = (badge, badgeImage) => {
        ...
        if (imageUrl) {
            return (
                <div className="flex justify-start items-center mt-2 mb-2 h-[28px]">
    ...
    const validRecommendations = recommendations.filter(product =>
        ...
        product.name &&
        product.slug &&
        (product.thumbnail !== undefined && product.thumbnail !== null) &&
