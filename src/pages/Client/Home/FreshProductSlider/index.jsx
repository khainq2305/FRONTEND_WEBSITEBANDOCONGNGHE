...
const InlinedProductCard = ({
    ...
    return (
        <div className="product-card-item w-full h-full flex flex-col border border-gray-200/70 rounded-lg overflow-hidden shadow-sm bg-white relative transition-shadow duration-200 ease-in-out group/productCard hover:shadow-md">
            ...
            {/* ✅ TẠO MỘT DIV MỚI ĐỂ BAO BỌC PHẦN HÌNH ẢNH */}
            <div className="relative">
                {/* Lớp phủ "Hết Hàng" sẽ nằm trong div này */}
                {!inStock && (
                ...
                {/* Huy hiệu giảm giá cũng nằm trong div này để bị lớp phủ che */}
                {discount > 0 && (
            ...
            {/* PHẦN THÔNG TIN VÀ NÚT BẤM NẰM HOÀN TOÀN BÊN NGOÀI, KHÔNG BỊ CHE */}
            <div className="product-card-info px-2 xs:px-1.5 sm:px-2.5 pt-1 pb-2 sm:pb-2.5 flex flex-col flex-grow overflow-hidden">
                <h3
  className="product-card-name font-semibold text-xs sm:text-[13px] text-gray-800 leading-tight mb-1 group-hover/productCard:text-blue-600 transition-colors duration-200 min-h-[38px]"
  title={name}
>
...
<div className="product-card-price text-[13px] sm:text-sm mb-0.5">
  {(price && parsePrice(price) > 0 && parsePrice(price) < parsePrice(oldPrice)) ? (
    <>
...
>
  {(price && parsePrice(price) > 0 && oldPriceNum > parsePrice(price))
    ? `Tiết kiệm ${(oldPriceNum - parsePrice(price)).toLocaleString('vi-VN')}₫`
    : ''}
</div>
                    ...
                    <div className="pt-1.5">
                        <div className="product-card-meta flex items-center justify-between mb-1.5 sm:mb-2 min-h-[18px]">
                            <div className="flex items-center gap-x-1 sm:gap-x-1.5 text-[10px] sm:text-[10.5px] text-gray-600">
                                ...
                                {rating > 0 && (
                                    <span className="text-gray-500">({parseFloat(rating).toFixed(1)})</span>
                                )}
                            </div>
    ...
    Đã bán {soldCount}
                        ...
                        <div className="product-card-actions flex items-center justify-between min-h-[26px]">
                            <button
                                ...
                                className="flex items-center gap-1 text-[10px] sm:text-xs text-gray-500 hover:text-blue-700 transition-colors focus:outline-none p-1 rounded hover:bg-gray-100"
                            >
                                <CompareIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                                <span className="leading-none whitespace-nowrap">So sánh</span>
                            </button>
                            <button
                                onClick={(e) => { e.stopPropagation(); onAddToFavorites(productId); }}
    ...
    const mappedBanners = (bannersFromProp || []).map((b, index) => ({
      ...
      link: b.linkValue
        ? b.linkType === 'URL' || (typeof b.linkValue === 'string' && b.linkValue.startsWith('http'))
          ? b.linkValue
         : b.linkType === 'product' 
 ? `/san-pham/${b.linkValue}` // linkValue ở đây là slug hoặc id của sản phẩm
 : b.linkType === 'category'
 ? `/danh-muc/${b.linkValue}` // linkValue ở đây là slug của danh mục
              : '#'
        : '#',
  ...
  const handleAddToFavorites = async (productId) => {
    ...
    setCurrentProducts((prev) => prev.map((p) => (p.productId === productId ? { ...p, isFavorite: !p.isFavorite } : p)));
  };
  ...
  return (
    <div className="fresh-slider-wrapper group max-w-[1200px] mx-auto bg-gray-50 rounded-lg shadow-md my-8">
      {/* ---- HEADER BLOCK - START ---- */}
      <div className="header-block px-3 sm:px-4 pt-4">
        <div className="flex flex-wrap justify-between items-center gap-y-2 mb-2 sm:mb-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-yellow-500 uppercase whitespace-nowrap">{sectionTitle}</h1>
        </div>
              ...
              <Slider {...topBannerSettings} ref={topBannerSliderRef} className="top-banner-slick-mobile">
                {banners.map((banner) => (
                  <div key={banner.id} className="px-0.5">
                    <a href={banner.link || '#'} className="block w-full rounded-md sm:rounded-lg overflow-hidden shadow-sm">
                      <img src={banner.imageUrl} alt={banner.altText} className="w-full h-auto object-contain" loading="lazy" />
                    </a>
                  </div>
                ))}
              </Slider>
