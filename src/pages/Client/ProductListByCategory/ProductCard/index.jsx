...
const handleCompare = (productId, name, thumbnail) => {
  ...
  return (
    <div className="w-full h-full flex flex-col border border-gray-200/70 rounded-lg overflow-hidden shadow-sm bg-white p-2.5 sm:p-3 relative transition-all duration-300 ease-in-out hover:shadow-xl hover:-translate-y-1 group">
      {discount && (
        <div className="absolute top-0 left-0 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-br-lg z-10">-{discount}%</div>
      )}
      ...
      <div className="mt-auto flex flex-col flex-grow justify-end">
        <div className="text-sm sm:text-[15px] mb-1">
          ...
          {oldPrice && <span className="text-gray-400 text-[10px] sm:text-xs line-through ml-1.5 sm:ml-2">{oldPriceFormatted}₫</span>}
        </div>
        {savings && <div className="text-green-600 text-[10px] sm:text-xs font-medium mb-1 sm:mb-2">Tiết kiệm {savings}₫</div>}
