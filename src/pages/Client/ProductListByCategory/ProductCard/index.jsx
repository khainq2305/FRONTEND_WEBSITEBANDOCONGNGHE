...
  image,
  rating,
//  badge, // ĐÃ BỎ PROP BADGE
  id, 
  isFavorite,
  onAddToFavorites,
  onCompare,
  inStock,   // Prop để xác định còn hàng / hết hàng / mới về
  soldCount, // Prop để hiển thị đã bán
}) {
  const renderStars = (rate) => {
    const stars = [];
    const numRating = parseFloat(rate);
    // Đảm bảo id được truyền vào để key của sao là unique nếu card này được dùng ở nhiều nơi
    const keyId = id || 'product'; // Fallback nếu id không có (mặc dù nên có)
    for (let i = 1; i <= 5; i++) {
      if (numRating >= i)
        stars.push(
          <FaStar key={`star-${i}-${keyId}`} className="text-yellow-400 text-xs" />
        );
...
      else
        stars.push(
          <FaRegStar key={`empty-${i}-${keyId}`} className="text-yellow-400 text-xs" />
        );
    }
    return stars;
  };

  // Bỏ hoàn toàn logic của badge
  // let badgeClasses = "...";
  // let badgeIcon = "...";
  // if (badge) { ... }
...
      <a href={`#product/${id}`} className="block"> {/* Link bao ảnh và tên */}
...
          />
        </div>
        {/* Bỏ hoàn toàn phần hiển thị badge */}
      <h3 
...
      <div className="mt-auto flex flex-col flex-grow justify-end"> {/* Đảm bảo actions luôn ở cuối */}
...
          )}
        
        {/* KHỐI HIỂN THỊ RATING VÀ TRẠNG THÁI "ĐÃ BÁN / HẾT HÀNG / MỚI VỀ" */}
...
          {/* Logic hiển thị "Đã bán", "Hết hàng", "Mới về" */}
...
          ) : ( // Mặc định nếu còn hàng và soldCount = 0 (hoặc không phải số) thì là "Mới về"
...
          <button 
            onClick={(e) => { e.stopPropagation(); if(onCompare) onCompare(id); }} 
...
          </button>
          <button 
            onClick={(e) => { e.stopPropagation(); if(onAddToFavorites) onAddToFavorites(id); }} 
...
          >
            {isFavorite ? <HeartSolidIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-red-500" /> : <HeartIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
