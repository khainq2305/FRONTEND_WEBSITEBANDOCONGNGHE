...
const AddressForm = ({ onSave, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await userAddressService.create({
        ...
        wardCode: selectedWard.code, // BE dùng code GHN
      });
  ...
  const boxClass = (hasErr, disabled) =>
    `w-full cursor-default border rounded-md bg-white py-2 pl-3 pr-10 text-left focus:outline-none ${
      disabled ? 'bg-gray-100' : 'focus:ring-1 focus:ring-sky-500'
