...
const OrderSummary = ({ totalAmount, discount, shippingFee, selectedPaymentMethod, selectedCoupon: propCoupon }) => {
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      if (selectedPaymentMethod === 2) {
        ...
        const updatedCart = fullCart.filter((cartItem) => !selectedItems.some((selected) => selected.skuId === cartItem.skuId));
  ...
  return (
    <div className="relative">
      <aside className="bg-white rounded-md p-3 sm:p-4 border border-gray-200 shadow-sm sticky top-6 h-fit">
        ...
        <div className="text-xs sm:text-sm text-gray-600 mb-4">
          ...
          <div className="flex justify-between mb-2">
            ...
            <span className="font-semibold text-red-600">-{totalDiscount.toLocaleString('vi-VN')} đ</span>
          </div>
          <div className="flex justify-between mb-2">
      ...
      {isPromoModalOpen && (
        <PromoModal onClose={() => setIsPromoModalOpen(false)} onApply={handleApplyPromo} appliedCode={selectedCoupon?.code || ''} />
      )}
    </div>
  );
};
