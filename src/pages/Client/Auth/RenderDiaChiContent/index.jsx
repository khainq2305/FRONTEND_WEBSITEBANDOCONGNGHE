...
//   return (
//     <div className={`p-4 sm:p-5 transition-all duration-200 ease-in-out ${isDefault ? 'bg-blue-50 dark:bg-blue-900/30' : 'bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
...
// <span className="text-xs text-gray-500 dark:text-gray-400 sm:border-l sm:border-gray-300 dark:sm:border-gray-600 sm:pl-2 sm:ml-1 flex items-center gap-1">
...
import { userAddressService } from '../../../../services/client/userAddressService'; // Đảm bảo đường dẫn đúng
import SuccessModal from '../SuccessModal'; // Đảm bảo đường dẫn đúng
...
const AddressPageContent = () => {
    ...
    useEffect(() => {
        const fetchAddresses = async () => {
            setLoading(true); // Start loading when fetching addresses
            try {
                const res = await userAddressService.getList();
                setAddresses(sortAddresses(res.data.data || []));
            } catch (err) {
    ...
    const handleOpenNewAddressModal = () => {
        setEditingAddress(null); // Ensure no editing address is set for new address
        setShowAddressModal(true);
    };

    const handleCloseAddressModal = () => {
        setShowAddressModal(false);
        setEditingAddress(null); // Clear editingAddress when modal closes
    };
    ...
    const handleSaveAddress = async (formDataToSave) => {
        ...
        const addressPayload = {
            ...
            isDefault: addresses.length === 0 ? true : addressData.isDefault, // Auto set default if first address
        ...
        try {
                    ...
                    sortAddresses(
                        prev.map((addr) =>
                            addr.id === editingAddress.id
                                ...
                                : addressPayload.isDefault && addr.isDefault // If updating and setting new default, unset old default
                                    ? { ...addr, isDefault: false }
                                    : addr
                        )
                    )
            ...
            console.error('Lỗi khi lưu địa chỉ:', err);
            if (err.response?.status === 400 && err.response?.data?.errors) {
                // You might want to pass these errors back to the modal to display there
                // For now, an alert will suffice as a fallback
    ...
    return (
        <div className="bg-white dark:bg-gray-900 p-4 sm:p-6 shadow-sm rounded-md border border-gray-200 dark:border-gray-700 min-h-[calc(100vh-150px)]">
            ...
            {loading && addresses.length === 0 ? ( // Only show full-screen loader when initially loading and no addresses
                <Loader fullscreen />
            ) : addresses.length > 0 ? (
                <div className="divide-y divide-gray-200 dark:divide-gray-700">
                    {addresses.map((addr) => (
