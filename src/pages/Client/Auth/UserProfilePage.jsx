...
const UserProfilePage = () => {
      ...
      try {
        ...
        if (response && response.data && response.data.user) {
          ...
          setSidebarUserInfo({
            ...
            displayName: apiUser.fullName || apiUser.email || 'Tài khoản', // Fallback nếu fullName không có
            avatarUrl: finalAvatarUrl, // Sử dụng URL đã xử lý
          });
        } else {
          // Trường hợp response không hợp lệ hoặc không có user data
  ...
  }, []); // Dependency array rỗng vì fetch chỉ chạy 1 lần khi mount

  // useEffect cho việc lắng nghe sự kiện avatarUpdated và profileUpdated có vẻ ổn.
  // Chỉ cần đảm bảo rằng `event.detail` (cho avatar) và `event.detail.user` (cho profile)
  // được `ProfileContent.jsx` gửi đi một cách chính xác.
  useEffect(() => {
    const handleAvatarUpdate = (event) => {
      // event.detail nên là URL đầy đủ đã xử lý hoặc null
      const newAvatarUrl = event.detail;
      // console.log("UserProfilePage - handleAvatarUpdate:", newAvatarUrl);
      setSidebarUserInfo((prev) => ({
        ...prev,
        avatarUrl: newAvatarUrl,
        // Cập nhật initial nếu avatar bị xóa và không có avatarUrl mới
        initial: newAvatarUrl ? (prev.displayName.charAt(0).toUpperCase() || '?') : (prev.displayName.charAt(0).toUpperCase() || '?') // Giữ initial cũ hoặc tính lại nếu cần
      }));
    };

    const handleProfileUpdate = (event) => {
      const updatedUserFromEvent = event.detail.user; // Đổi tên để tránh nhầm lẫn
      // console.log("UserProfilePage - handleProfileUpdate:", updatedUserFromEvent);
      if (updatedUserFromEvent) {
        setSidebarUserInfo((prev) => ({
          ...
          // Cập nhật avatarUrl từ profileUpdated nếu có và nếu khác biệt
          // (Tuy nhiên, avatarUpdated thường xử lý riêng việc này tốt hơn)
          // avatarUrl: updatedUserFromEvent.avatarUrl ? (logic xử lý URL tương tự fetchSidebar) : prev.avatarUrl 
        }));
      }
    };
  ...
  }, []); // Chạy một lần để đăng ký listeners


  // ... (phần còn lại của UserProfilePage.jsx giữ nguyên) ...
...
            <div className="overflow-hidden">
                  {/* Kiểm tra isSidebarLoading trước khi hiển thị displayName */}
