...
const CartItem = ({ item, isChecked, onToggleChecked, onQuantityChange, isLastItem }) => {
  ...
  return (
    <div className={`bg-white ${!isLastItem ? 'border-b border-gray-200' : ''} ${isOutOfStock ? 'bg-gray-50' : ''}`}>
      {(isDeleting || isUpdating || isSelecting) && <OrderLoader fullscreen />}
      ...
      <div className="flex flex-col p-3 sm:hidden">
        <div className="flex items-start w-full">
          {isOutOfStock ? (
            ...
            <div
              className={`w-5 h-5 mt-1 flex items-center justify-center rounded-sm transition border ${isChecked ? 'bg-primary border-primary text-white' : 'bg-white border-gray-500 text-transparent'
                } cursor-pointer text-xs font-bold flex-shrink-0 mr-3`}
              onClick={async () => {
                if (isSelecting || isDeleting || isUpdating || isOutOfStock) return;
                try {
                  setIsSelecting(true);
                  ...
                  toast.error('Không thể cập nhật trạng thái chọn!', { position: 'top-right' });
      ...
      <div className="hidden sm:grid sm:grid-cols-[40px_minmax(0,1fr)_140px_100px_140px_40px] sm:items-center sm:px-4 sm:gap-x-0 py-3 min-h-[100px]">
        {isOutOfStock ? (
          ...
          <div
            className={`w-5 h-5 flex items-center justify-center rounded-sm transition border ${isChecked ? 'bg-primary border-primary text-white' : 'bg-white border-gray-500 text-transparent'
