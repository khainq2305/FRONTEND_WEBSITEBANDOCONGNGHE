...
// Icons (StarIcon, CameraIcon, CloseIcon - giữ nguyên như trước)
...
const CloseIcon = ({ className, onClick }) => (
  <svg className={className} onClick={onClick} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
  </svg>
);


export default function ReviewFormModal({ productName, show, onClose, onSubmitReview }) {
  ...
  const handleSubmit = () => {
    ...
    if (comment.trim().length > 0 && comment.trim().length < 15 ) { 
        alert("Nhận xét phải có ít nhất 15 kí tự (nếu có nhập).");
        return;
    }
    onSubmitReview({ rating, comment, productName });
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="p-4 md:p-6 overflow-y-auto">
          ...
          <div className="text-xs text-center h-6 mt-1 mb-3 flex items-center justify-center">
            <span className={`truncate transition-colors duration-150 ${labelColorClass}`}>
              {currentLabel}
            </span>
          </div>
          ...
          <textarea
            ...
            rows="4"
            placeholder="Xin mời chia sẻ một số cảm nhận về sản phẩm (nhập tối thiểu 15 kí tự nếu có)"
            value={comment}
            onChange={(e) => setComment(e.target.value)}
          ></textarea>

          <button className="mt-4 flex items-center gap-2 text-sm text-primary hover:text-blue-700 border border-gray-300 px-3 py-2 rounded-md hover:bg-blue-50 transition-colors">
            <CameraIcon className="w-5 h-5" />
            Thêm hình ảnh
          </button>
        </div>
          
        <div className="p-4 md:p-6 border-t border-gray-200 sticky bottom-0 bg-white z-10">
          <button 
            onClick={handleSubmit}
            className="w-full bg-primary text-white py-2.5 px-4 rounded-md font-semibold hover:bg-opacity-80 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 transition-all duration-150 text-sm"
            // Nếu bạn có class hover-primary định nghĩa sẵn cho hiệu ứng hover (ví dụ làm tối màu nền):
            // className="w-full bg-primary text-white py-2.5 px-4 rounded-md font-semibold hover-primary focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 transition-all duration-150 text-sm"
          >
            GỬI ĐÁNH GIÁ
          </button>
        </div>
