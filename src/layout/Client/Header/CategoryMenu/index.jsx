...
const CategoryMenu = ({ topLevelCategories = [], allCategories = [], isOpen = false }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            if (topLevelCategories.length > 0) {
                ...
                // Nếu danh mục đang hover là loại 'post' hoặc không có con nào được hiển thị, set level2Display rỗng
                // Logic để xác định liệu có con hay không đã được xử lý ở Header.jsx (chỉ product có children)
                // và được lọc ở đây.
    ...
    useEffect(() => {
        if (activeL1Category && isOpen) {
            // Nếu danh mục đang hoạt động là loại 'post' hoặc không có con nào được hiển thị, set level2Display rỗng
    ...
    // Hàm để tạo đường dẫn động dựa trên 'type' của danh mục
                    ...
                    {topLevelCategories.map((cat) => (
                        <li key={cat.id} className="relative">
                            ...
                            >
                                ...
                                {/* CHỈ HIỂN THỊ MŨI TÊN NẾU KHÔNG PHẢI LÀ LOẠI 'post' VÀ CÓ DANH MỤC CON */}
