...
useEffect(() => {
        const fetchCombinedCategories = async () => {
            try {
                // Gọi API getCombinedMenu mới
                const response = await categoryService.getCombinedMenu();
                const combinedNested = response.data || [];

                // Hàm đệ quy để làm phẳng và chuẩn hóa dữ liệu
                const flattenAndStandardize = (items, parentId = null) => {
                    return items.flatMap(item => {
                        const standardizedItem = {
                            id: item.id, // ID đã có tiền tố từ backend (e.g., "product-1", "post-201")
                            name: item.name,
                            slug: item.slug,
                            parent_id: parentId, // parent_id của item hiện tại (null cho cấp 1)
                            thumbnail: item.thumbnail,
                            type: item.type, // 'product' hoặc 'post'
                        };
