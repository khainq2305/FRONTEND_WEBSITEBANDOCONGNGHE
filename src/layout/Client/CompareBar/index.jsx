...
import ProductSearchModal from './ProductSearchModal'; // Đảm bảo đường dẫn đúng
...
const [toastMessage, setToastMessage] = useState(null);
  ...
  const handleCompareClick = () => {
    if (filteredCompareItems.length >= 2) {
      // Lấy topLevelId của sản phẩm đầu tiên để kiểm tra đồng nhất
      ...
      if (allSameTopLevelCategory) {
        ...
        // Có thể thêm thông báo cho người dùng ở đây nếu muốn
      }
    } else {
  ...
  const handleOpenModal = (index) => {
    ...
    // Khi mở modal, truyền category ID của sản phẩm đã có (nếu có)
    // Hoặc nếu không có sản phẩm nào, có thể để null để modal tự xử lý
    const firstCompareItem = filteredCompareItems[0];
    setTargetCategoryId(firstCompareItem?.category?.id || null); // Vẫn truyền category ID cấp con cho modal để lọc search results
    setIsModalOpen(true);
  };

  const handleSelectProduct = (product) => {
    // Logic này hiện tại đã được chuyển vào handleCompareButtonClick trong ProductSearchModal
    // nhưng nếu bạn muốn giữ lại xử lý thêm/thay thế ở đây (ví dụ: khi thêm từ trang khác không qua modal)
    // thì cần cập nhật logic tương tự như handleCompareButtonClick trong modal.
    // Hiện tại, modal đang gọi addToCompare/removeFromCompare trực tiếp.
    // Hàm này (handleSelectProduct) có vẻ không còn được gọi từ ProductSearchModal
    // vì ProductSearchModal gọi handleCompareButtonClick trực tiếp.
    // Nếu bạn muốn modal chỉ trả về sản phẩm và CompareBar xử lý logic thêm/xóa,
    // thì cần thay đổi cách ProductSearchModal gọi onSelectProduct.
    // Với cấu trúc hiện tại, logic so sánh danh mục chính nằm trong ProductSearchModal.
  ...
  // Không render nếu không có sản phẩm và thanh đang thu gọn
  if (filteredCompareItems.length === 0 && isCompareBarCollapsed) return null;
  // Hoặc nếu modal đang mở, chỉ render modal (đã chuyển logic này ra ngoài return của CompareBar)


  return (
    <>
      {/* ProductSearchModal được render conditionally bên ngoài CompareBar chính */}
    <ProductSearchModal
  isOpen={isModalOpen}
  onClose={resetModal}
  ...
  showToast={(msg) => setToastMessage(msg)} // ✅ thêm dòng này
/>
     ...
     {!isModalOpen && !isCompareBarCollapsed && (
        ...
        <div className="fixed bottom-0 left-0 right-0 max-w-[1200px] mx-auto bg-white border-t border-gray-200 shadow-lg z-[9999] p-0"
          ...
          {[0, 1, 2].map((index) => (
            <div key={index} className="relative flex-1 border-r border-gray-300 flex flex-col items-center justify-center p-2 text-center h-full">
              {compareItems[index] ? (
                <>
                  ...
                  <img src={compareItems[index].image} alt={compareItems[index].name}
                    className="max-h-[70%] max-w-full object-contain mb-1" />
                  <div className="text-xs px-1 max-w-full overflow-hidden text-ellipsis whitespace-nowrap">
                    {compareItems[index].name}
                  </div>
                </>
              ) : (
